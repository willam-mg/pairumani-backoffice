...
class RestauranteProductoFormRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        if (routerequest('productos_create')) {
            $rules['nombre'] = ['required', 'max:50', 'unique:restaurante_productos'];
            $rules['foto'] = ['required', 'mimes:jpeg,png,jpg,gif,svg', 'max:2048'];
            // $rules['restaurante_categoria_id'] = ['required','integer'];
        }

        if (routerequest('productos_edit')) {
