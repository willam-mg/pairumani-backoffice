                ...
                <div class="card">
                    <div class="card-header card-header-icon card-header-rose">
                        ...
                        <div style="text-align: right;padding-top: 15px;">
                            @if(kvfj(Auth::user()->rol->permisos,'cafeteria_opciones_create'))
                                <a class="btn btn-success" href="{{ route('cafeteria_opciones_create',[$categoria->id,$producto->id]) }}" title="Nueva Opcion">
                            ...
                            <div class="col-7">
                                <table class="table">
                                    <tbody>
                                        <tr>
                                            <td>
                                                @include('cafeteriaopciones.search')
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
									...
									@if(count($opciones))
										@foreach ($opciones as $opcion)
											<tr>
												...
												<td>
                                                    @if(kvfj(Auth::user()->rol->permisos,'cafeteria_opciones_show'))
                                                        <a href="{{ route('cafeteria_opciones_show',[$categoria->id,$producto->id,$opcion->id]) }}" class="btn btn-info btn-round btn-just-icon" title="Detalle opcion">
                                                            <i class="material-icons">visibility</i>
                                                        </a>
                                                    @endif
													@if(kvfj(Auth::user()->rol->permisos,'cafeteria_opciones_edit'))
														<a name="editar" href="{{ route('cafeteria_opciones_edit',[$categoria->id,$producto->id,$opcion->id]) }}" class="btn btn-warning btn-round btn-just-icon" title="Editar opcion">
															<i class="material-icons">mode_edit</i>
														</a>
													@endif
													@if(kvfj(Auth::user()->rol->permisos,'cafeteria_opciones_destroy'))
											...
											@include('cafeteriaopciones.modal')
										@endforeach
									@else
										<tr>
